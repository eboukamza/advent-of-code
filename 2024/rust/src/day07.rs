...
pub fn read_lists() -> u64 {
    ...
    if let Ok(lines) = common::read_lines("./input8.txt") {
        for line in lines.flatten() {
            let values = line.split(": ").collect::<Vec<&str>>();
            let result = values[0].parse::<u64>().unwrap();
            let left_values: Vec<_> = values[1].split(" ").map(|x| x.parse::<u64>().unwrap()).collect::<Vec<_>>();
...
fn find_result_concat(result: u64, partial: u64, list: &Vec<&u64>) -> bool {
    ...
    find_result_concat(result, new_partial_1, &new_list) || find_result_concat(result, new_partial_2, &new_list) || find_result_concat(result, new_partial_3, &new_list)
}
